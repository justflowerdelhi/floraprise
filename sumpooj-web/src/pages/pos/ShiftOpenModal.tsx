...
const ShiftOpenModal: React.FC = () => {
  const { activeShift, loading, error, openShift, shiftSystemAvailable } = useShift();
  ...
  // Don't show if shift system isn't available (API not deployed), already open, or still loading
  if (!shiftSystemAvailable || loading || activeShift) return null;
