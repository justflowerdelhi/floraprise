...
void main() {
  ...
  test('Cloud-selected product UUID is submitted without a SQLite product mapping', () async {
    final db = await AppDatabase.instance.database;
    final productId = await _insertProduct();
    await _insertInventory(productId, currentQty: 5);
    final orderId = await _insertDraft(productId: productId);
    const cloudProductId = '22222222-2222-4222-8222-222222222222';
    ...
    final payload = await _orderRepository().buildCloudPosSalePayload(
      orderId: orderId,
      clientSyncId: '33333333-3333-4333-8333-333333333333',
      orderNo: 'ORD-POS-33333333-3333-4333-8333-333333333333',
    );

    final line = (payload['lines'] as List).single as Map<String, dynamic>;
    expect(line['cloudProductId'], cloudProductId);

    await _orderRepository().finalizeCloudConfirmedDraft(
      orderId: orderId,
      orderNo: 'ORD-POS-33333333-3333-4333-8333-333333333333',
      cloudOrderId: '44444444-4444-4444-8444-444444444444',
    );
